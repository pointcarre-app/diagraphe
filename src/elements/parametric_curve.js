...
export class ParametricCurve extends CartesianElement {
   ...
   * @param {string} [options.dasharray] - SVG stroke-dasharray for dashed lines // ADDED
   */
  constructor(options) {
    super(options);
    ...
    this.dasharray = options.dasharray ?? null; // ADDED
  }
  ...
  renderShape() {
    ...
    // ADDED: Apply dasharray if provided
    if (this.dasharray) {
...
export class Function extends ParametricCurve {
   ...
   * @param {number} [options.strokeWidth=2] - Curve stroke width
   * @param {string} [options.fill="none"] - Fill color (usually "none" for curves)
   * @param {string} [options.dasharray] - SVG stroke-dasharray for dashed lines // ADDED (inherited from parent)
   */
  constructor(options) {
